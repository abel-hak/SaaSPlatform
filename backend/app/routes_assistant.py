...
def get_conversation_messages(
    ...
    conv = db.query(Conversation).filter(Conversation.id == conversation_id, Conversation.org_id == org.id).first()
    if not conv:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Conversation not found")
    msgs = (
        db.query(Message)
    ...
    return schemas.ConversationMessagesResponse(conversation_id=conversation_id, messages=msgs)
