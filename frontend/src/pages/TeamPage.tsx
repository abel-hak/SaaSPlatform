    ...
    <div className="space-y-6">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Manage workspace members and roles. Seats are enforced per plan.</p>
        </div>
        <button
            ...
            {!pageLoading && (!data || data.members.length === 0) && (
              <tr>
                <td colSpan={4} className="py-12 text-center">
                  ...
                  <p className="text-sm text-slate-400 dark:text-[#8e8e8e]">No members yet.</p>
                </td>
              </tr>
            )}
          </tbody>
        ...
        <div className="dialog-overlay" onClick={() => setInviteOpen(false)}>
          <div className="dialog-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="invite-email" className="block text-sm font-medium text-slate-700 dark:text-[#d4d4d4] mb-1.5">
                  Email
                </label>
                <input
                  id="invite-email"
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button onClick={() => setInviteOpen(false)} className="btn-secondary text-sm py-2 px-4">
                  Cancel
                </button>
                <button disabled={loading} onClick={() => void invite()} className="btn-primary text-sm py-2 px-4">
                  Send invite
                </button>
              </div>
            </div>
